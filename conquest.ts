...
class Objective {
    ...
    setProgressVisuals(progress: number): void {
        ...
        this.uiPosition = mod.CreateVector(mod.Add(-110, mod.Floor(mod.Divide(width, 2))), 200, 0);
    }
}
...
const objectives: Array<Objective | null> = [
    ...
    new Objective(5, 205, "F", mod.VoiceOverFlags.Foxtrot, 650, 651),
    new Objective(6, 206, "G", mod.VoiceOverFlags.Golf, 660, 661),
    new Objective(7, 207, "H", mod.VoiceOverFlags.Hotel, 670, 671),
    new Objective(8, 208, "I", mod.VoiceOverFlags.India, 680, 681),
    new Objective(9, 209, "J", null, 690, 691),
    new Objective(10, 210, "K", null, 700, 701),
    new Objective(11, 211, "L", null, 710, 711),
    new Objective(12, 212, "M", null, 720, 721),
    new Objective(13, 213, "N", null, 730, 731),
    new Objective(14, 214, "O", null, 740, 741),
    new Objective(15, 215, "P", null, 750, 751),
    new Objective(16, 216, "Q", null, 760, 761),
    new Objective(17, 217, "R", null, 770, 771),
    new Objective(18, 218, "S", null, 780, 781),
    new Objective(19, 219, "T", null, 790, 791),
    new Objective(20, 220, "U", null, 800, 801),
    new Objective(21, 221, "V", null, 810, 811),
    new Objective(22, 222, "W", null, 820, 821),
    new Objective(23, 223, "X", null, 830, 831),
    new Objective(24, 224, "Y", null, 840, 841),
    new Objective(25, 225, "Z", null, 850, 851)
];
...
class UIController {
    updatePlayerScoreboard(player: mod.Player): void {
        mod.SetScoreboardPlayerValues(
            player,
            getPlayerState(player).score,
            getPlayerState(player).kills,
            getPlayerState(player).deaths,
            getPlayerState(player).assists,
            getPlayerState(player).captures
        );
    }
    ...
    setupMainUI(): void {
        ...
        mod.AddUIText(
            ...
            mod.FindUIWidgetWithName("container"),
            true,
            0,
    ...
    setupScoreUI(leftScore: string, rightScore: string, leftBar: string, rightBar: string, team: mod.Team): void {
        mod.AddUIText(
            ...
            0.8,
            mod.UIBgFill.Blur,
            mod.Message("{}", getTeamState(team).score),
            32,
            friendlyTextColour,
            1,
            mod.UIAnchor.Center,
        ...
        mod.AddUIText(
            ...
            0.8,
            mod.UIBgFill.Blur,
            mod.Message("{}", getTeamState(getTeamState(team).otherTeam).score),
            32,
            enemyTextColour,
            1,
            mod.UIAnchor.Center,
        ...
        mod.AddUIText(
            leftBar,
            mod.CreateVector(mod.Add(-260, mod.Divide(mod.Multiply(200, mod.Divide(getTeamState(team).score, getTeamState(team).startingScore)), 2)), 60, 0),
            mod.CreateVector(mod.Multiply(200, mod.Divide(getTeamState(team).score, getTeamState(team).startingScore)), 10, 0),
            mod.UIAnchor.TopCenter,
            mod.FindUIWidgetWithName("container"),
            true,
            0,
        ...
        mod.AddUIText(
            ...
            mod.CreateVector(
                mod.Subtract(
                    ...
                    mod.Divide(
                        mod.Multiply(
                            200,
                            mod.Divide(getTeamState(getTeamState(team).otherTeam).score, getTeamState(getTeamState(team).otherTeam).startingScore)
                        ),
                        2
                    )
                ),
            ...
            mod.CreateVector(
                mod.Multiply(200, mod.Divide(getTeamState(getTeamState(team).otherTeam).score, getTeamState(getTeamState(team).otherTeam).startingScore)),
                10,
                0
            ),
            mod.UIAnchor.TopCenter,
    ...
    setupPlayerUI(player: mod.Player): void {
        ...
        mod.AddUIText(
            "OOBText",
            mod.CreateVector(0, 470, 0),
            mod.CreateVector(450, 150, 0),
            mod.UIAnchor.TopCenter,
            playerRoot,
            false,
            1,
        ...
        mod.AddUIText(
            ...
            0,
            mod.UIBgFill.None,
            mod.Message("{}", playerState.outOfBoundsCountdown),
            72,
            enemyTextColour,
            1,
    ...
    showVersion(): void {
        mod.AddUIText(
            ...
            1,
            mod.UIBgFill.None,
            mod.Message("ViperStudiosAndy | andy6170 | Conquest Template V11"),
            12,
            mod.CreateVector(1, 1, 1),
            0.5,
            mod.UIAnchor.Center,
...
class PlayerController {
    ...
    shouldExitAreaTrigger(eventInfo: { eventPlayer: mod.Player; eventAreaTrigger: mod.AreaTrigger }): boolean {
        ...
        return (
            triggerId === CONFIG.INVISIBLE_WALL_TRIGGER_ID ||
    ...
    async enterInvisibleWallTrigger(eventInfo: { eventPlayer: mod.Player; eventAreaTrigger: mod.AreaTrigger }): Promise<void> {
        ...
        playerState.lastRotation =
            mod.RoundToInteger(mod.XComponentOf(rotation)) === 0
        ...
        const movementDirection =
            mod.DistanceBetween(playerState.lastMovement, ZERO_VECTOR) > 0 ? mod.Normalize(playerState.lastMovement) : mod.ForwardVector();
        mod.Teleport(player, mod.Add(playerState.lastPosition, mod.Multiply(movementDirection, playerState.pushBackDistance)), playerState.lastRotation);
    }
    ...
    async applyRepelForce(time: number, eventInfo: { eventPlayer: mod.Player; eventInteractPoint: mod.InteractPoint }): Promise<void> {
        ...
        if (mod.YComponentOf(repelPosition) > mod.YComponentOf(playerPosition)) {
            const targetPosition = mod.Add(
                mod.CreateVector(mod.XComponentOf(playerPosition), mod.YComponentOf(repelPosition), mod.ZComponentOf(playerPosition)),
                mod.UpVector()
            );
            mod.SetObjectTransformOverTime(player, mod.CreateTransform(targetPosition, targetRotation), time, false, false);
        } else {
            const targetPosition = mod.CreateVector(mod.XComponentOf(repelPosition), mod.YComponentOf(playerPosition), mod.ZComponentOf(repelPosition));
    ...
    async handleTeamSwitchAndRepel(eventInfo: { eventPlayer: mod.Player; eventInteractPoint: mod.InteractPoint }): Promise<void> {
        ...
        if (FLAGS.ENABLE_TEAM_SWITCHING && (mod.Equals(mod.GetInteractPoint(998), interactPoint) || mod.Equals(mod.GetInteractPoint(999), interactPoint))) {
        ...
        if (interactPointId >= 700 && interactPointId < 750) {
            ...
            const repelForce = mod.DistanceBetween(mod.GetObjectPosition(player), mod.GetObjectPosition(repelOrigin)) / 8;
    ...
    async handleDeath(eventInfo: PlayerCombatEventInfo): Promise<void> {
        ...
        if (mod.IsPlayerValid(closestTeammate) && mod.DistanceBetween(playerPosition, mod.GetObjectPosition(closestTeammate)) > 20) {
            await mod.Wait(3);
...
class CapturePointController {
    ...
    async onCaptured(eventInfo: CapturePointEventInfo): Promise<void> {
        ...
        uiController.updateScoreboard();
        const playersOnObjective = filterModArray(mod.GetPlayersOnPoint(eventInfo.eventCapturePoint), (currentArrayElement: any) =>
            mod.Equals(mod.GetTeam(currentArrayElement), mod.GetCurrentOwnerTeam(eventInfo.eventCapturePoint))
        );
        ...
        if (FLAGS.ENABLE_VO) {
            ...
            if (objective?.voiceFlag !== null && objective?.voiceFlag !== undefined) {
                ...
                mod.PlayVO(
                    ...
                    objective.voiceFlag,
                    getTeamState(mod.GetCurrentOwnerTeam(eventInfo.eventCapturePoint)).otherTeam
                );
            }
        }
    }
    ...
    async runProgressLoop(eventInfo: CapturePointEventInfo): Promise<void> {
        while (!isGameOngoing) {
            await waitUntil(999, () => isGameOngoing, 1);
        }
        if (FLAGS.CONQUEST_ASSAULT) {
    ...
    async onLostNeutralised(eventInfo: CapturePointEventInfo): Promise<void> {
        ...
        const playersOnObjective = filterModArray(mod.GetPlayersOnPoint(eventInfo.eventCapturePoint), (currentArrayElement: any) =>
            mod.Equals(mod.GetTeam(currentArrayElement), neutralisingTeam)
        );
        for (let i = 0; i < mod.CountOf(playersOnObjective); i++) {
    ...
    refreshCaptureUIForPlayerObjective(player: mod.Player): void {
        ...
        uiController.manageCapturePointUI(playerState.currentCapturePoint, playerState.capturePointState, {
...
class AIController {
    ...
    shouldTargetOnKillAssist(eventInfo: PlayerCombatEventInfo): boolean {
        return (
            getPlayerState(eventInfo.eventPlayer).isCustomAI &&
    ...
    async readyForAttack(eventInfo: PlayerEventInfo): Promise<void> {
        ...
        while (mod.IsPlayerValid(player)) {
            if (canEngageNearbyEnemy()) {
                ...
                if (mod.IsPlayerValid(closestEnemy) && mod.DistanceBetween(mod.GetObjectPosition(closestEnemy), playerPosition) < 25) {
    ...
    async enterVehicle(eventInfo: { eventPlayer: mod.Player; eventVehicle: mod.Vehicle }): Promise<void> {
        ...
        if (mod.IsPlayerValid(eventInfo.eventPlayer)) {
            if (isPlayerInVehicle(eventInfo.eventPlayer)) {
                if (mod.LessThan(mod.DistanceBetween(mod.GetObjectPosition(eventInfo.eventPlayer), getPlayerState(eventInfo.eventPlayer).startPosition!), 3)) {
    ...
    spawnAIObjectives(eventInfo: PlayerEventInfo): void {
        ...
        if (!isPlayerInVehicle(player)) {
            for (const objective of objectives) {
                ...
                if (
                    mod.Equals(playerTeam, mod.GetCurrentOwnerTeam(capturePoint)) &&
                    (!mod.IsPlayerValid(closestEnemy) || mod.DistanceBetween(mod.GetObjectPosition(closestEnemy), capturePointPosition) > 40)
                ) {
    ...
    async tryDeployNearbyAIVehicle(distance: number, eventInfo: PlayerEventInfo): Promise<void> {
        ...
        const nearbyOpenVehicles = filterModArray(
            mod.AllVehicles(),
            (vehicle: any) =>
                mod.CountOf(mod.GetAllPlayersInVehicle(vehicle)) < 2 &&
                mod.DistanceBetween(mod.GetVehicleState(vehicle, mod.VehicleStateVector.VehiclePosition), playerPosition) < distance
        );
    ...
    deployAIVehicle(vehicle: mod.Vehicle | null | undefined, distance: number, eventInfo: PlayerEventInfo): void {
        ...
        if (mod.IsPlayerValid(player) && !isPlayerInVehicle(player)) {
            ...
            const playerPosition = mod.GetObjectPosition(player);
            if (isDefined(vehiclePosition) && isDefined(playerPosition) && mod.DistanceBetween(vehiclePosition, playerPosition) < distance) {
    ...
    addAI(): void {
        ...
        const team1PlayerCount = mod.CountOf(filterModArray(players, (player: any) => mod.Equals(mod.GetTeam(player), team1)));
        const team2PlayerCount = mod.CountOf(filterModArray(players, (player: any) => mod.Equals(mod.GetTeam(player), team2)));
...
class ConquestGame {
    ...
    shouldEndGame(): boolean {
        return isGameOngoing && (mod.GetMatchTimeRemaining() <= 1 || getTeamState(TEAM_1).score <= 0 || getTeamState(TEAM_2).score <= 0);
    }
    ...
    updateNearEndMusic(): void {
        ...
        if (mod.GreaterThan(mod.Multiply(mod.Divide(60, mod.GetMatchTimeRemaining()), 4), endMusicIntensity)) {
            mod.SetMusicParam(mod.MusicParams.Core_Urgency, mod.Subtract(4, mod.Multiply(mod.Divide(mod.GetMatchTimeRemaining(), 60), 4)));
        } else {
    ...
    shouldPlayVOWinning(): boolean {
        return isGameOngoing && FLAGS.ENABLE_VO && mod.GreaterThan(getTeamState(TEAM_1).score, getTeamState(TEAM_2).score);
    }
    ...
    shouldPlayVOTeam2Winning(): boolean {
        return isGameOngoing && FLAGS.ENABLE_VO && mod.GreaterThan(getTeamState(TEAM_2).score, getTeamState(TEAM_1).score);
    }
    ...
    checkConquestAssaultWin(): void {
        ...
        this.updateOwnedFlagCounts();
        const team2AlivePlayerCount = mod.CountOf(
            filterModArray(
                mod.AllPlayers(),
    ...
    async resetFX(eventInfo: PlayerEventInfo): Promise<void> {
        ...
        for (let i = 2000; i < 2999; i++) {
            ...
            mod.EnableVFX(mod.GetVFX(i), true);
            if (mod.Equals(mod.RoundToInteger(mod.Modulo(i, 5)), 0)) {
    ...
    async endGame(): Promise<void> {
        ...
        if (mod.LessThan(getTeamState(TEAM_1).score, 0)) {
            getTeamState(TEAM_1).score = 0;
        }
        if (mod.LessThan(getTeamState(TEAM_2).score, 0)) {
            getTeamState(TEAM_2).score = 0;
        }
        if (mod.GreaterThan(getTeamState(TEAM_1).score, getTeamState(TEAM_2).score)) {
            mod.SetMusicParam(mod.MusicParams.Core_IsWinning, 1, mod.GetTeam(1));
        } else if (mod.GreaterThan(getTeamState(TEAM_2).score, getTeamState(TEAM_1).score)) {
